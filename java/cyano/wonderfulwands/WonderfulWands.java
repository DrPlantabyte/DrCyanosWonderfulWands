...
public class WonderfulWands {
    ...
    @EventHandler public void preInit(FMLPreInitializationEvent event)
    {
		...
		boolean isServer = event.getSide() == cpw.mods.fml.relauncher.Side.SERVER;
		wizardHat = new WizardsHat(serverSafeRenderIndex("wizhat",isServer));
		GameRegistry.registerItem(wizardHat, wizardHat.itemName);
		witchHat = new WitchsHat(serverSafeRenderIndex("witchhat",isServer));
		GameRegistry.registerItem(witchHat, witchHat.itemName);
		topHat = new TopHat(serverSafeRenderIndex("tophat",isServer));
    ...
    private int serverSafeRenderIndex(String prefix, boolean isServer){
    	if(isServer){
    		return 0;
    	} else {
